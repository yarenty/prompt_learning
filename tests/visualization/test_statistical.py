...
def control_treatment_data():
    ...
    np.random.seed(42)
    control = {"Dataset 1": np.random.normal(100, 15, 50).tolist(), "Dataset 2": np.random.normal(95, 15, 50).tolist()}
    treatment = {
...
def test_plot_confidence_intervals(visualizer, sample_data):
    ...
    path = visualizer.plot_confidence_intervals(sample_data, confidence_levels=[0.80, 0.90, 0.95])
    assert path.exists()

    # Test with single dataset
    path = visualizer.plot_confidence_intervals({"Single": sample_data["Method A"]})
    assert path.exists()
    ...
    with pytest.raises(ValueError):
        visualizer.plot_confidence_intervals(sample_data, confidence_levels=[0.5, 1.5])  # Invalid levels
...
def test_plot_effect_sizes(visualizer, control_treatment_data):
    ...
    mismatched_treatment = {"Dataset 1": treatment["Dataset 1"], "Dataset 3": treatment["Dataset 2"]}  # Different key
    ...
    with pytest.raises(ValueError):
        visualizer.plot_effect_sizes({"Invalid": []}, {"Invalid": [1, 2, 3]})  # Empty list
...
def test_plot_power_analysis(visualizer):
    ...
    path = visualizer.plot_power_analysis(effect_size=0.3, alpha=0.01, power_target=0.90)
    assert path.exists()
    ...
    with pytest.raises(ValueError):
        visualizer.plot_power_analysis(effect_size=0.5, alpha=1.5)  # Invalid alpha

    # Test with invalid power target
    with pytest.raises(ValueError):
        visualizer.plot_power_analysis(effect_size=0.5, power_target=1.5)  # Invalid power
...
def test_plot_customization(visualizer, sample_data):
    ...
    path = visualizer.plot_confidence_intervals(sample_data, title="Custom Title")
    assert path.exists()

    # Test with custom colors
    path = visualizer.plot_confidence_intervals(sample_data, marker=dict(color="red"))
    assert path.exists()

    # Test with custom size
    path = visualizer.plot_confidence_intervals(sample_data, width=800, height=600)
    assert path.exists()

    # Test with custom theme
    viz = StatisticalVisualizer(output_dir=visualizer.output_dir, theme="dark")
    path = viz.plot_confidence_intervals(sample_data)
    assert path.exists()
...
def test_error_handling(visualizer, sample_data):
    ...
    with pytest.raises(ValueError):
        visualizer.plot_confidence_intervals({"Invalid": ["not", "numbers"]})
