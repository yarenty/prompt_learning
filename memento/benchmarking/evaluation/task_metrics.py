...
class ProgrammingMetrics:
    ...
    def calculate_pass_at_k(self, generated_code: str, test_cases: List[Dict[str, Any]], k: int = 1) -> float:
    ...
    def calculate_runtime_metrics(self, code: str, test_inputs: List[Any]) -> Dict[str, float]:
...
"""
                ...
                # Run the test
                result = subprocess.run(["python", f.name], capture_output=True, text=True, timeout=self.test_timeout)
...
"""
                ...
                if time_match and memory_match:
                    return {"time": float(time_match.group(1)), "memory": float(memory_match.group(1))}
