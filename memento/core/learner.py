...
class PromptLearner:
    ...
    def evolve_prompt(self, current_prompt: str, lessons: List[Dict]) -> str:
        """
        ...
        try:
            response = ollama.generate(model=self.model, prompt=evolution_prompt)
    ...
    def save_evolution_step(
        ...
        file_path = (
            self.storage_path
            / f"evolution_{prompt_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def get_evolution_history(self, prompt_type: Optional[str] = None) -> List[Dict]:
        ...
        if prompt_type:
            return [
                ...
                if step["prompt_type"] == prompt_type
            ]
