...
@click.option(
    "--datasets",
    multiple=True,
    default=["humaneval", "gsm8k", "writingbench"],
    help="Datasets to benchmark on"
)
@click.option(
    "--models", 
    multiple=True,
    default=["memento", "promptbreeder", "self_evolving_gpt"],
    help="Models to evaluate"
)
@click.option(
    "--baseline",
    default="promptbreeder",
...
@click.option(
    "--max-samples",
    type=int,
    help="Maximum samples to validate per dataset"
)
    ...
    for dataset_name in datasets:
        ...
        try:
            ...
            if hasattr(dataset, '__iter__'):
                ...
                if first_item:
                    click.echo(f"  📋 Sample keys: {list(first_item.keys()) if isinstance(first_item, dict) else 'Non-dict item'}")
...
def _create_config_template(template: str) -> dict:
    ...
    if template == "advanced":
        base_config.update({
            "datasets": [
                "humaneval", "apps", "livecodebench",
                "gsm8k", "math_hard", "mmlu_math",
                "writingbench", "creativity"
            ],
        ...
        base_config.update({
            "datasets": [
                "humaneval", "bigcodebench", "apps", "livecodebench",
                "gsm8k", "math_hard", "mmlu_math",
                "writingbench", "creativity", "biggen_bench"
            ],
            "models": ["memento", "promptbreeder", "self_evolving_gpt", "auto_evolve"],
            "baseline": "promptbreeder",
                ...
                for dataset_improvements in improvements.values():
                    if isinstance(dataset_improvements, dict):
                        all_improvements.extend([
                            v for v in dataset_improvements.values() 
